<?php
		...
		if (isset($_POST['submit']))
		{
			...
			if (!is_numeric($_POST['clasa']) || (is_numeric($_POST['clasa']) && ($_POST['clasa'] < 5 || $_POST['clasa'] > 12)) && $este_valid != 0)
				{
					$este_valid = 0;
					echo "<br/><br/>Clasa invalidă (ia mâna de pe Inspect Element)";
				}
			...
			if (verify($_POST['judet']) == false && $este_valid != 0)
				...
				if ($este_valid == 1)
				{
					...
					else
					{
						...
						if ($clasa != 0)
						{
							$query = "INSERT INTO users (username, password, puncte, clasa, nume, prenume, scoala, localitate, judet) VALUES (?, SHA(?), 0, ?, ?, ?, ?, ?, ?)";

							$statement = $databaseConnection->prepare($query);
							$statement->bind_param('ssdsssss', $username, $password, $clasa, $nume, $prenume, $scoala, $localitate, $judet);
							$statement->execute();
						...
						{
							echo "<br/><br/>Nu ai selectat clasa!";
						}
					}
				}
			else
...
?>
	<div id="continutul_nostru">
		...
		<div class="row-fluid">
			<div class="span12">
			<p><a href="register_prof.php">Dacă vrei să te înregistrezi ca profesor, click aici.</a></p>
				<form action="register.php" method="post">
            <fieldset>
                <legend>Înregistrare cont elev</legend>
                <ul type="none">
                    <li>
